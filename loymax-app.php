...
if ( ! class_exists( 'LoymaxWebApp_Plugin' ) ) {
    class LoymaxWebApp_Plugin {
        ...
        /**
         * Перенапрявляет на следующий шаг помощника установки
         */
        ...
        private function add_actions() {
            ...
            add_action( 'init', function () {
                // При обновлении плагина делаем бэкап кастомных шаблонов
                ...
                // Обработка запросов при настройке плагина
                ...
                // Обработка запроса на отображение помощника установки
            ...
            // При завершении обновления плагина возвращаем кастомные шаблоны обратно
            ...
            add_action( 'before_delete_post', function ( $post_id ) {
                // Обрабатываем удаление страницы ЛК
                if ( $this->loymax_page_ID && $post_id == strval ( $this->loymax_page_ID ) ) {
                    ...
                    exit();
                } elseif ( get_option( 'loymax-navigation-menu-id' ) ) { // Обрабатываем удаление пункта меню
            ...
            // Обрабатываем изменение ссылки на страницу ЛК
            ...
            // Если есть опция loymax_install_wizard_in_progress - отображаем сообщение для запуска помощника установки
        ...
        /**
         * Получает из БД компонент по идентификатору пункта меню
         * @param $menu_item_id
        ...
        /**
         * Обновляет ссылку страницы ЛК
         * @param int $post_id идентификатор страницы
         */
        ...
        /**
         * Отображает сообщение о невозможности удаления страницы ЛК
         */
        ...
        /**
         * Отображает сообщение о невозможности удаления меню для страницы ЛК
         */
        ...
        /**
         * Задает домен для перевода настроек плагина
         */
        ...
        /**
         * Перемещает кастомные шаблоны
         * @param bool $restore если false - создается бэкап файлов, если true - файлы восстанавливаются из бэкапа
         */
        ...
        private function create_updater() {
            ...
            $this->updater = new LoymaxWebApp_updater();
        }
        ...
        private function generate_app_config( $is_wpglobus_active ) {
            ...
            <script type="text/javascript">
                (function ( e ) {
                    ...
                    <?php if ( ! empty( $this->custom_templates ) ): ?>
                    e.lmxConfig.templatesPath = "<?php echo( $custom_templates_url ); ?>";
                    e.lmxConfig.customTemplates = <?php echo( wp_unslash( json_encode( $this->custom_templates ) ) ); ?>;
                    <?php endif;
        ...
        private function include_scripts( $wpglobus_helper ) {
            // Скрипт для обновления состояния меню ЛК в реальном времени (SPA)
            ...
            // Стили для управления внешним видом меню на основе классов на body (добавляются динамически, через SPA)
            wp_enqueue_style( 'loymax-web-app-up-styles', plugins_url( 'includes/css/lmxWebAppUPStyles.css', __FILE__ ), array(), md5( microtime() ) );

            $deps = array();
            // Если активен плагин WP_Globus - добавляем его как зависимость для инклюда
            if ( $wpglobus_helper->get_is_wpglobus_active() ) {
                $deps[] = 'wpglobus';
                $wpglobus_helper->add_inline_script();
            }
            // UP-Приложение и его стили
            ...
            if ( is_dir( $custom_templates_path ) ) {
                ...
                foreach ( $iterator as $file_info ) {
                    if ( $file_info->getExtension() === 'html' ) {
                        array_push( $this->custom_templates, str_replace( $custom_templates_path . '/', '', wp_normalize_path( $file_info->getPathname() ) ) );
                    }
                }
            }
        }
        ...
        /**
         * Получает из БД таблицу с настройками
         * @param bool|null $redirect_options_only если значение не null - получает только опции для редиректов
         * @return array|object|null
         */
        ...
        /**
         * Обновляет в БД таблицу с настройками
         */
         ...
         * @param bool $is_error отобразить ли уведомление с типом "Ошибка"
         * @param string $content содержимое уведомления
         */
