...
/**
 * Класс для работы с БД при обновлении или установке плагина
 */
class LoymaxWebApp_updater {
    ...
    /**
     * Обрабатывает событие активации плагина
     */
    private function register_activation_hooks() {
        register_activation_hook( LOYMAX_WEB_APP_PLUGIN_FILE, array( $this, 'create_options_table' ) );
        register_activation_hook( LOYMAX_WEB_APP_PLUGIN_FILE, array( $this, 'create_components_table' ) );
        register_activation_hook( LOYMAX_WEB_APP_PLUGIN_FILE, array( $this, 'update_tables' ) );
        register_activation_hook( LOYMAX_WEB_APP_PLUGIN_FILE, array( $this, 'start_install' ) );
    }
     ...
     * @param bool $is_success признак успеха транзакции
     */
    ...
    /**
     * Создает в БД таблицу компонентов
     */
    ...
    /**
     * Заполняет значениями из конфига таблтцу опций в БД
     */
    ...
    /**
     * Заполняет значениями из конфига таблтцу компонентов в БД
     */
    private function init_components_table() {
        ...
        foreach ( $this->configs_default->loymax_components as $key => $config ) {
            ...
            $result = $this->wpdb->update(LOYMAX_WEB_APP_COMPONENT_TABLE_NAME, $info, array('c_key' => $key));
            if ($result === FALSE || $result < 1) {
                $this->wpdb->insert(LOYMAX_WEB_APP_COMPONENT_TABLE_NAME, $info);
            }
        }
        $result = $result !== false;
        return $result;
    ...
    /**
     * Обновляет данные в БД, если установленная версия отличается от версии в БД
     */
    ...
    /**
     * Устанавливает признак необходимости отображения помощника установки
     */
    ...
    /**
     * Обновляет данные в БД до установленной версии
     */
    public function update() {
        ...
        if ( $result !== false && $this->installed_version === 2.9 ) {
            // Добавляем столбец в таблицу настроек
        ...
        if ( $result !== false && $this->installed_version === 3 ) {
            $result = $this->update_to_version( '3.1' );
            if ( $result !== false ) {
                $this->installed_version = 3.1;
            }
        }

        // Если при обновлении БД произошла ошибка отображаем сообщение об лшибке
        if ( $result === false ) {
    ...
    /**
     * Выполняет обновление БД до заданной версии
    ...
    /**
     * Заполняет значениями из конфига столбец component в таблтце опций в БД
     */
    ...
    /**
     * Обновляет опции редиректа до принятого в версии 3 формата
     */
